...
public class ThreadGroupUpdatePanel extends TablePanel
{
    ...
    private void showThreadGroup(int p_index)
    {
        if(p_index != -1)
        {
            Item x_threadItem = (Item) ThreadGroupHelper.getAllActiveUpdates(o_threadGroup).get(p_index);
    ...
    void tableRowDoubleClicked(int col, int row)
    {
        switch(col)
        {
			case 1: showThreadGroup(row); break;
