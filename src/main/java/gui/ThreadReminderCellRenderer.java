...
class ThreadReminderCellRenderer extends DefaultTableCellRenderer {
    ...
    public Component getTableCellRendererComponent(JTable p_table, Object p_value, boolean p_isSelected, boolean p_hasFocus, int p_row, int p_col) {
        Component x_component = super.getTableCellRendererComponent(p_table, p_value, p_isSelected, p_hasFocus, p_row, p_col);
		setBorder(noFocusBorder);

		x_component.setForeground(Color.black);
		x_component.setBackground(Color.white);

		List<Reminder> x_reminders = getAllActiveReminders(o_thread, o_onlyDueReminders);

		if(p_row < x_reminders.size()) {
			x_component.setBackground(getColourForTime(x_reminders.get(p_row).getDueDate()));
		}
